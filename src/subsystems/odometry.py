...
class Odometry(Subsystem):
    ...
    def __init__(self, drive: Drive, vision: Vision, gyro: AnalogGyro):
        super().__init__()
        self.drive = drive
        self.vision = vision
        self.gyro = gyro
        ...
        self.pose_estimator = MecanumDrivePoseEstimator(
            ...
            wheelPositions=self.drive.get_wheel_positions(),
            initialPose=Pose2d(
    ...
    def periodic(self) -> None:
        ...
        new_pose = self.pose_estimator.update(
            self.gyro.getRotation2d(), self.drive.get_wheel_positions()
        )
